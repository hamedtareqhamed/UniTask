        ...
        try {
            for (appWidgetId in appWidgetIds) {
                val views = RemoteViews(context.packageName, R.layout.countdown_widget).apply {
                    ...
                    val room = widgetData.getString("next_class_room", "") ?: ""
                    ...
                    val details = if (section.isNotEmpty()) "$type - $section" else type
                    setTextViewText(R.id.widget_class_details, details)
                    
                    setTextViewText(R.id.widget_time, time)
                    setTextViewText(R.id.widget_room, room)
                    
                    if (countdown != "N/A" && countdown.isNotEmpty()) {
            ...
            android.util.Log.e("UniTask", "Error updating countdown widget", e)
        }
    }
}
